...
/**
 * Computes CPU usage from two `sysinfo.jiffies` readings.
 *
 * The router reports the same ten space-separated counters Linux keeps in the
 * `cpu` line of `/proc/stat` (user, nice, system, idle, iowait, irq, softirq,
 * steal, guest, guest_nice), as a single space-separated string rather than a
 * parsed structure. Usage is only meaningful between two samples, the same way
 * a load average needs no baseline but a percentage does; this mirrors the
 * formula `status-overview.asp` itself uses to render its own CPU bar
 * (`jiffiesusage = (1000*(diff_total-diff_idle)/diff_total)/10`), confirmed
 * against a live router rather than assumed from the reference implementation.
 *
 * @param {string|null|undefined} previous - `sysinfo.jiffies` from the previous poll
 * @param {string|null|undefined} current - `sysinfo.jiffies` from this poll
 * @returns {number|null} Usage in percent, rounded to one decimal, or null if it cannot be derived
 */
function cpuUsageFromJiffies(previous, current) {
	/**
	 * @param {string|null|undefined} raw - Space separated jiffy counters
	 * @returns {{total: number, idle: number}|null} Parsed totals
	 */
	const parse = raw => {
		...
		// fields[3] is idle jiffies, the fourth column of /proc/stat's cpu line.
		return { total: fields.reduce((sum, n) => sum + n, 0), idle: fields[3] };
	};
	...
	const diffTotal = to.total - from.total;
	const diffIdle = to.idle - from.idle;
	// A counter that did not advance, or went backwards (a reboot between
	// polls), leaves nothing to divide by.
	if (diffTotal <= 0) {
		return null;
	}
 ...
 */
const INFLUX_GROUPS = {
	system: id => /^system\.(cpuTemperature|cpuUsage|load1|load5|load15|memFree|memAvailable|memTotal)$/.test(id),
	...
	deviceCounts: id =>
		id === 'devices.count' ||
		id === 'devices.wirelessCount' ||
		id === 'devices.onlineCount' ||
		id === 'devices.offlineCount',
};
